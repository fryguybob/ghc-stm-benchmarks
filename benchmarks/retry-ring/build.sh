...
# for n in fine-hle; do
    ...
    cabal install optparse-applicative stm ../throughput/ --with-ghc $ghc
    
    cabal exec -- $ghc -O2 -threaded -rtsopts -eventlog Main.hs -outputdir .build-$n -fno-omit-yields -o Main-$n
    cabal exec -- $ghc -O2 -threaded -rtsopts -eventlog MainB.hs -outputdir .buildB-$n -fno-omit-yields -o MainB-$n
