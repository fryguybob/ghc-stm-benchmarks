...
newMVar :: a -> IO (MVar a)
newMVar x = atomically (newTVar (Just x))

takeMVar :: MVar a -> IO a
takeMVar ptr = atomically (
   do v <- readTVar ptr
      case v of
        Nothing -> retry
        Just x -> do writeTVar ptr Nothing
                     return x )

putMVar :: MVar a -> a -> IO ()
putMVar ptr new = atomically (
   do v <- readTVar ptr
      case v of
        Nothing -> writeTVar ptr (Just new)
        Just _ -> retry )
...
data List a = Node { val :: a
                   , next :: MVar (List a) }
            | Null
            | Head { next :: MVar (List a) } deriving Eq
