...
data Treap k where
    Treap :: mutable Node k -> IO (Treap k)

hasValue :: Ord k => k -> Treap k -> IO Bool
hasValue k (Treap n) = do
    (l,e,g) <- readRef n >>= split k
    merge3 l e g >>= writeRef n
    return $ isNode e

insert :: Ord k => GenIO -> k -> Treap k -> IO ()
insert gen k (Treap n) = do
    (l,e,g) <- readRef n >>= split k
    e' <- if isNode e then return e else mkNode gen k
    merge3 l e' g >>= writeRef n

erase :: Ord k => k -> Treap k -> IO ()
erase k (Treap n) = do
    (l,e,g) <- readRef n >>= split k
    merge l g >>= writeRef n
          ...
          | otherwise    = do
            ...
            case a of
                ...
                2 -> do
                    b <- hasValue cur' t
                    if b
                      then loop (i+1) cur' (res + 1)
                      else loop (i+1) cur' res
    loop 0 5 0 >>= print
