...
# for n in fine-hle; do
    ...
    if [ $n == "pastm-tl2" ] ; then
        ...
        cabal install optparse-applicative-0.12.1.0 stm-2.4.3 unordered-containers-0.2.4.0 \
            transformers-compat-0.4.0.4 tagged-0.7.2 \
            $bc ../throughput/ ../random/pcg-random/ ./ $d \
            --disable-executable-stripping --with-ghc $ghc --ghc-options="$dump"
    fi
