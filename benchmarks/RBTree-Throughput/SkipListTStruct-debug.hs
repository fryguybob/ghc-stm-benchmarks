...
unsafeReadNode' e Nil a = error $ "unsafeReadNode Nil! (" ++ e ++ ")"
unsafeReadNode' _ n   a = unsafeReadNode n a
  ...
  return r
    where
      ...
      buildPrevs node lvl prevs = do
        ...
        next <- unsafeReadNode' "insert, buildPrevs" node l
        if isNil next
      ...
      insertNode nodesHeight prevs = do
        ...
        let updatePtrs lvl _ 
            ...
            updatePtrs lvl (p:ps) = do
                nextNode <- unsafeReadNode' "insert, updatePtrs" p lvl
...
get (SkipList headNodes height states) k = do
    loop height headNodes
       where
        ...
        loop lvl nodes = do
           let !l = lvl - 1
           next <- unsafeReadNode' "get, loop" nodes l
           if isNil next
             then loop l nodes
             else do
...
delete sl@(SkipList headNodes height states) k = do
  ...
  where
    ...
    loop lvl node = do
      ...
      -- start there.
      next <- unsafeReadNode' "delete, loop" node l
      if isNil next
    ...
    unlink prev' node lvl = go lvl prev'
      where 
        ...
        findAndWrite l prev = do
          n <- unsafeReadNode' "delete, findAndWrite" prev l
          if n == node
            then do
              next <- unsafeReadNode' "delete, swing, next" node l
...
validate (SkipList headNode height states) = do
    ...
    forM_ ns $ \node -> do
        forFirstNil node $ \i ->
            forM_ [i..levels node - 1] $ \j -> do
                n <- unsafeReadNode' ("All Nil above " ++ show i ++ " failed at " ++ show j) node j
  ...
  where
    ...
    getLevel l node = do
        n <- unsafeReadNode' ("getLevel " ++ show l) node l
        if isNil n
          then return []
          else (n :) <$>  getLevel l n
    ...
    forFirstNil node act = do
        let l = levels node
            go i | i >= l    = return ()
                 | otherwise = do
              n <- unsafeReadNode' ("forFirstNil" ++ show i) node i
              if isNil n
                then act i
                else go (i+1)
        go 0
...
delete' (SkipList headNodes height states) k = do
  ...
  where
    ...
    loop lvl nodes = do
      let !l = lvl - 1
      next <- unsafeReadNode' "delete, loop" nodes l
      if isNil next
        then loop l nodes
        else do
          k' <- readKeyP next -- Non-transactional
    ...
    swingPtrs lvl nodes = do
      let !l = lvl - 1
      next <- unsafeReadNode' "delete, swing, next" nodes l
      nextNext <- unsafeReadNode' "delete, swing, nextNext" next l
