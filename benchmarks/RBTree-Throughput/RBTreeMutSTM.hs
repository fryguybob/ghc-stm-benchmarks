...
fixAfterInsertion s x = do
    ...
    ro <- readTRef (root s)
    c <- readTRef (color ro)
    when (c /= Black) $ writeTRef (color ro) Black
  where
    loop Nil = return ()
    loop x = do
        sr <- readTRef (root s)
...
fixAfterDeletion tree x = do
    ...
    when (isNode x') $ do
      c <- readTRef (color x')
      when (c /= Black) $ writeTRef (color x') Black
  where
    loop x = do
...
verifyRedBlack n   d = do
  ...
  where
    ...
    isBlack n   = readTRef (color n) >>= \c -> return (c == Black)
