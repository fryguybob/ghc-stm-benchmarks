...
flattenLevel :: Event -> [Event]
flattenLevel (Event _ (EventBlock _ _ es)) = es
flattenLevel e = [e]
...
lengthT :: (Num t, Ord t) => Transaction t -> t
lengthT (T b e) = e - b
...
main = do
    ...
    analyse ts (takeBaseName f)

processFile f = do
    (Right el) <- readEventLogFromFile f
    return . concatMap transactions . map (mapMaybe toTrans) . map flattenLevel . events . dat $ el
--    return . filter isValid . transactions . mapMaybe toTrans . concatMap flatten . events . dat $ el
  where
    isValid (T b e) = b < e
...
analyse ts name = do
    ...
    when True $ printTable ["Average", "Min", "Max", "Std Dev", "Samples"]
      (map (flip map ss)
         [ (^.ave.to (TCom 2))
         , (^.minValue.to (TCom 2))
         , (^.maxValue.to (TCom 2))
    ...
    forM_ ts $ \t -> do
        let l = lengthT t
        when (l > 10000000000) $ putStrLn $ "Large: " ++ show t
        when (l < 0) $ putStrLn $ "Small: " ++ show t
