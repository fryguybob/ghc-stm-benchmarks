...
newtype Bucket k v = Bucket { _unBucket :: Array (k, v) }
type BucketArray k v = TArray Int (Bucket k v)
...
emptyBucket = unsafePerformIO $ IO $ \s1# ->
    case newArray# 0# undefined s1# of
      (# s2#, ma# #) -> 
        case unsafeFreezeArray# ma# s2# of
          (# s3#, a# #) -> (# s3#, Bucket (Array a#) #)
{-# NOINLINE emptyBucket #-}

bucketSize :: Bucket k v -> Int
bucketSize b = sizeofArray (_unBucket b)
{-# INLINE bucketSize #-}

readData :: Bucket k v -> Int -> (k, v)
readData b i = indexArray (_unBucket b) i
{-# INLINE readData #-}

insertBucket :: Bucket k v -> k -> v -> STM (Bucket k v)
insertBucket b k v = do
    a <- growArray (_unBucket b) (k,v)
    Bucket <$> unsafeFreezeArray a
{-# INLINE insertBucket #-}

removeBucket :: Bucket k v -> Int -> STM (Bucket k v)
removeBucket b i = do
    a <- shrinkArray (_unBucket b) i
    Bucket <$> unsafeFreezeArray a
{-# INLINE removeBucket #-}
        ...
        then return True
          ...
          search iSet k >>= \case
            Just idx -> do
              ...
              removeBucket iSet idx >>= update i t hi
              let jsz = bucketSize jSet
              if | jsz < _threshold t -> insertBucket jSet k' v' >>= update j t hj >> return True
                 | jsz < _probeCap  t -> insertBucket jSet k' v' >>= update j t hj 
                                                            >> go (round-1) (1-i) (1-j) hj
                 | otherwise          -> insertBucket iSet k' v' >>= update j t hj >> return False
...
remove t k = do
    ...
    search set0 k >>= \case
      Just idx -> removeBucket set0 idx >>= update 0 t h0 >> return True
      Nothing  -> do
        search set1 k >>= \case
          Just idx -> removeBucket set1 idx >>= update 1 t h1 >> return True
